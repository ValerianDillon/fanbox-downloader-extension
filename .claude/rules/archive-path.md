...
- 投稿ディレクトリ: `<タイトル> [<postId>]`。タイトルが空に潰れる場合は `[<postId>]` だけ
- カバーと本文アセット: `001<拡張子>` から始まる数字連番。カバーがあれば先頭に置き、桁数は最低 3 桁
- 生成ファイル: `index.html` と `post.json`。選択した添付だけの場合も `post.json` は入れる

数字だけの stem は投稿ディレクトリ直下の予約名 (`index.html` / `post.json`) と一致しない。
`post.json` は postId、creatorId、タイトル、原 URL、公開・更新日時、投稿種別、タグ、本文の保存状態、含めたアセットと archive 内のファイル名を持つ。

**postId と拡張子は `~` を escape 記号にして符号化する。**
postId は利用者が付けたタイトルと同じディレクトリ名へ入るので、正規化で別の ID が同じ名前へ潰れない形にする。
...
**数字連番は AssetKey と保存実績で凍結する。**
現在の投稿から消えたアセットの番号も予約し、新しいアセットへ再利用しない。
再利用すると、別のアセットが過去の ZIP と同じ番号を指してしまう。
凍結済みの番号が 3 桁を超える場合は、その最大番号に合わせて新しい名前の桁数を広げる。
...
タイトルと postId、数字連番と拡張子の組み立てだけで単射を保証しきるのは難しく (`encodeFileName` の潰し、凍結名は任意の値を取れる)、共有層はアセット同士の名前衝突を検査しない。
...
識別子を先に確保し、残りを投稿タイトルに割り当てる。
投稿タイトルは `encodeFileName` と `%` / `?` の全角化を済ませてから残りのバイト予算で切る。
正規化前に切ると、`/` のような 1 byte の文字が全角 3 bytes へ膨らんで最終セグメントの上限を超えるためである。
