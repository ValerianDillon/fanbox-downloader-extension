...
- **両者の形式が常に違えば、省略は一度も起こらない。** 実害は無い (毎回取得する) が、機能が黙って効かない状態になる。実機テストで一致を確認すること (`.claude/skills/extension-live-test/SKILL.md`)

## 収集の直前に読み直す

**画面が持っている履歴は収集に使わない。** 収集を始める直前に `chrome.storage.local` から読み直す。
同じ creator を別のタブで開いて記録を削除しても、こちらのメモリ上の履歴は消えないので、削除したはずの履歴を根拠に `post.info` を省いてしまう (確認文の「次回は全件を取得します」に反する)。
...
- 完全な現在の snapshot が必要なら「前回保存分も取得する」を使う
...
- **前回保存分も取得する** — 省略だけを止める。凍結名は据え置く (「再取得する」と「過去に割り当てた名前を忘れる」は別の要求)
