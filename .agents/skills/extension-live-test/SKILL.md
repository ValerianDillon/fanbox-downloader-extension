...
| 実行方法 | `bun run test:smoke` (自動) | ランチャー起動 + agent-browser で手動操作 |
...
- `agent-browser`: ランチャーの CDP endpoint に接続し、ページの移動、アクセシビリティスナップショット、クリック、入力、ページ側のログや通信を操作する CLI
- `scripts/live-cdp-eval.ts`: agent-browser が対象にしない拡張 service worker を生 CDP で直接調べる CLI
...
3. 名前付きセッションで agent-browser を CDP endpoint に接続して操作する
   - 接続と初回スナップショット: `agent-browser --session fanbox-live --cdp 9222 snapshot -i`
   - 移動: `agent-browser --session fanbox-live --cdp 9222 open 'https://www.fanbox.cc/@<creatorId>'`
   - DOM や画面状態が変化したら `snapshot -i` を撮り直す。`@e1` のような参照は直近のスナップショットに属するため、過去の参照を使い回さない
   - 収集対象は 1〜2 投稿など最小限に絞る (「安全上の注意」参照)
4. agent-browser のセッションを `agent-browser --session fanbox-live close` で切断してから、`pkill -TERM -f "[l]ive-browser.ts"` を送る。`context.close()` が実行されプロファイルに状態が保存されてから終了する
...
2. 拡張の FAB ボタン (画面右下 `bottom: 24px; right: 24px` に固定表示、`src/content/fab.ts`) をクリックする。通常ビルドの shadow root は closed のため DOM セレクタでは shadow 内に届かない。closed shadow root でも accessibility tree には露出するため、`snapshot -i` を撮ると FAB は `button "⬇"` として `@eN` 参照付きで現れる。表示された参照を `agent-browser --session fanbox-live --cdp 9222 click @eN` に渡す
...
4. 保存ステップ (`pickSaveHandle` → `showSaveFilePicker`, `src/content/downloader.ts`) はネイティブのファイル保存ダイアログを要求するため、headless では完走できない。実測では headless Chromium の `showSaveFilePicker` は即座に `AbortError` を投げる。保存先の取得は収集後の review で「ダウンロード開始」を押した時点なので、通常ビルドの headless でも収集、選択 UI、投稿情報の export までは確認できる。保存まで確認したい場合は headed で行う

### headless で ZIP 生成まで観測したい場合はテストビルドを使う

通常ビルドの headless では上記のとおり review の picker が abort するため、実 API を相手にした収集は観測できるが、メディア取得と完了画面の分岐までは進めない。ZIP フェーズまで headless で観測したいときはテストビルド (`__FBDL_TEST__=true`) を一時的に `dist/` へ置く。
...
- 手順: `bun run build:test` → `cp -a dist-test/. dist/` → ランチャー起動。観測後は `bun run build` で通常ビルドを再生成して必ず戻す (`scripts/build.ts` が出力先を掃除してから生成するため、手動で `dist/` を削除する必要はない)
...
- `--remote-debugging-port` のポートが既に使用中 (bind エラー) → 前回のランチャーが残っていないか `pgrep -af "[l]ive-browser.ts"` で確認し (ブラケットを外すと呼び出し元のシェル自身の argv にマッチして偽陽性になる)、残っていれば SIGTERM で止める。無関係なプロセスが占有している場合は勝手に kill せず、「ポートを変更する場合」(下記 agent-browser の準備と接続確認の節) に従って別ポートで起動する
...
### Windows 側 Vivaldi の remote inspect を使う場合

ユーザ自身のターミナルからも headed 起動できず、Windows から agent 環境の仮想 NIC へ到達できる場合は、headless Chromium の CDP を Vivaldi DevTools の screencast から操作できる。
CDP へ接続したプロセスはログイン済みブラウザを全面的に操作できるため、接続先は仮想 NIC の特定アドレスだけに限定し、ログイン完了後すぐ中継を止める。

1. headless ランチャーを永続プロファイル付きで起動し、agent-browser から FANBOX のログインボタンを押して pixiv Accounts のログインページまで進める。
2. `hostname -I` とネットワーク構成から Windows が到達できる IPv4 アドレスを 1 個選び、`<agent-ip>` とする。Tailscale、Docker bridge、loopback は選ばず、判断できなければユーザへ確認する。
3. `socat TCP-LISTEN:<port>,bind=<agent-ip>,reuseaddr,fork TCP:127.0.0.1:<port>` を foreground で起動する。`0.0.0.0` には bind しない。
4. agent 側で `curl -fsS http://<agent-ip>:<port>/json/version` を実行し、`webSocketDebuggerUrl` の host が `<agent-ip>:<port>` になっていることを確認する。
5. ユーザに Windows 側 Vivaldi で `vivaldi://inspect/#devices` を開いてもらい、`Discover network targets` の `Configure...` に `<agent-ip>:<port>` を追加してもらう。
6. `Login | pixiv` の `inspect` を開き、DevTools のコマンドメニューから `Show Screencast` を実行してログインしてもらう。
7. FANBOX に戻ったことを確認したら socat に Ctrl+C を送り、`ss -ltnp` で `<agent-ip>:<port>` の待受が消えたことを確認する。Vivaldi の discovery 設定からも対象を外してよい。

`agent-browser dashboard start` は代替候補だが、起動成功の表示後にプロセスが終了してポートが待ち受けない環境がある。
ユーザへ dashboard URL を案内する前に、`ss -ltnp` と `curl -fsS http://127.0.0.1:4848/` の両方で稼働を確認する。
...
生のレスポンス形状を観測したいときは、host_permissions を持つ拡張の service worker コンテキストで evaluate する。agent-browser はページ操作に使い、service worker には `bun scripts/live-cdp-eval.ts sw '<expression>'` で直接接続する。この CLI は service worker target の webSocketDebuggerUrl に生の WebSocket で接続し、`Runtime.evaluate` を送る。
...
**agent-browser の `network requests` に拡張の API 要求は出ない。** content script の fetch は service worker のプロキシを通るため、ページのネットワークログにはページ自身の要求しか現れない。「2 回目の収集で `post.info` が減るか」のように発行数そのものを検証したいときは使えない。

数えるときは service worker 側の `fetch` を包んで記録する。ランチャーを起動した後、収集を始める前に 1 回だけ流す。

```
bun scripts/live-cdp-eval.ts sw "(()=>{if(globalThis.__fbdlProbe)return{already:true};globalThis.__fbdlProbe=[];const orig=globalThis.fetch;globalThis.fetch=function(...a){const u=typeof a[0]==='string'?a[0]:a[0]?.url;if(typeof u==='string'&&u.includes('api.fanbox.cc'))globalThis.__fbdlProbe.push(u);return orig.apply(this,a);};return{installed:true};})()"
```

読み出しは `bun scripts/live-cdp-eval.ts sw "globalThis.__fbdlProbe"`、計測の区切りでのリセットは `globalThis.__fbdlProbe.length=0`。

- 記録するのは URL だけで、拡張の挙動は変えない
- **service worker が再起動するとパッチごと消える。** 計測の直前に `!!globalThis.__fbdlProbe` を確かめ、消えていれば入れ直す
- ZIP フェーズのメディア取得は `api.fanbox.cc` を通らないのでこの記録には現れない (そちらは `fbdlMediaAttempts`)
...
- `data-fbdl-zip-b64` が publish されるのは ZIP が `ZIP_B64_PUBLISH_LIMIT` (8 MiB、`src/content/test-hooks.ts`) 以下のときだけで、超えると `zip-url` (Blob URL) と `zip-size` のみになる。取り出しは ZIP サイズによらず `bun scripts/live-pull-zip.ts <out.zip>` を使う。8 MiB 以下でも agent-browser の evaluate 結果としてインラインに受け取らない
...
## agent-browser の準備と接続確認

この環境では mise の npm backend で agent-browser をユーザー全体に導入する。
初回だけ次を実行する。
...
このマシンでは導入と Chrome for Testing の取得まで完了している。
ランチャーの起動後、`agent-browser --session fanbox-live --cdp 9222 snapshot -i` がページのアクセシビリティスナップショットを返せば接続できている。
ページの診断には同じ `--session` と `--cdp` を付けて `console`、`errors`、`network requests --type xhr,fetch` を使う。
拡張 service worker の通信はページ側の通信一覧に含まれないため、`scripts/live-cdp-eval.ts` で調べる。

### agent-browser を実行できない場合

`mise which agent-browser` と `agent-browser --version` でインストール状態を確認する。
CLI を利用できない状態でページ操作を別の自動化手段へ暗黙に切り替えない。
生 CDP (`curl http://127.0.0.1:<port>/json/version`, `/json/list`) で起動と拡張のロードまで確認し、ページ操作が未検証であることを報告する。

### ポートを変更する場合

- 代替ポートは `ss -ltn` で空きを確認してから 9223, 9224, … の順に採番する
- ランチャーに `--port <port>`、agent-browser と `scripts/live-cdp-eval.ts` にも同じポートを渡す。永続的な接続登録の変更は不要である
